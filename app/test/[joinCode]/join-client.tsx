...
export default function JoinClient({ test }: { test: TestWithRelations }) {
  ...
  const handleStartTest = async (data?: ParticipantFormValues) => {
    // For authenticated users, no form data needed
    ...
    try {
      const result = await createParticipant({
        testId: test.id,
        username: !isAuthenticated && data ? data.username : undefined,
      });
