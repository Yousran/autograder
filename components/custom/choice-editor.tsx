...
/** Serialize a Plate Value to the string stored in `choiceText`. */
function serializeValue(value: Value): string {
  return JSON.stringify(value);
}

/**
 * Recursively extract all plain text from a Plate node tree.
 * Used to validate that the editor content is not empty.
 */
function extractText(nodes: Value): string {
  return nodes
    .map((node) => {
      ...
      return Array.isArray(children) ? extractText(children as Value) : "";
    })
    .join("");
}

/**
 * Returns the plain-text content of a serialized Plate JSON string.
 * Falls back to the raw string when it is not valid JSON.
 */
export function getChoiceEditorPlainText(serialized: string): string {
  try {
    const nodes = JSON.parse(serialized) as Value;
    if (Array.isArray(nodes)) return extractText(nodes);
  } catch {
    // not JSON
  }
  return serialized;
 ...
 * cause expensive re-renders. Callback props are always up-to-date via
 * internal refs, so they are excluded from the equality check.
 */
export const ChoiceEditor = memo(
  function ChoiceEditor({
    ...
    useEffect(() => {
      saveValueRef.current = async (serialized: string) => {
        ...
        // Skip when both the new content and the initial value represent the
        // same plain text (e.g. empty Plate JSON vs. empty string "").
        const plainText = getChoiceEditorPlainText(serialized).trim();
        const initialPlainText = getChoiceEditorPlainText(
          initialValueRef.current,
        ).trim();
        if (plainText === initialPlainText) return;
        try {
    ...
    return (
      <TooltipProvider>
        <Plate editor={editor} onChange={handleChange}>
          <div
            className="flex-1 min-w-0 rounded-md border border-input ring-offset-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2"
            onBlur={handleBlur}
          >
            <FixedToolbar className="sticky top-0 overflow-x-auto scrollbar-hide rounded-t-md border-b">
              {/* Undo / Redo */}
              <ToolbarGroup>
                <UndoToolbarButton />
                <RedoToolbarButton />
  ...
  // Only re-render when content-related props change.
  // onUpdate / onUpdateError are always fresh via internal refs.
  (prev, next) =>
